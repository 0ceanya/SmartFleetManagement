// src/app/orders/receipts/[invoiceId]/page.tsx
"use client";
...
import type { Receipt, InvoiceSummary } from "@/lib/types";
    ...
    async function load() {
      ...
      try {
        const [receiptData, invoiceData] = await Promise.all([
          apiFetch<Receipt>(`/api/billing/invoices/${invoiceId}/receipt`),
          apiFetch<InvoiceSummary>(`/api/billing/invoices/${invoiceId}`),
        ]);
        if (!cancelled) {
      ...
      {!loading && !error && receipt && (
        <div className="bg-white border border-gray-200 rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="px-8 py-6 space-y-4">
            ...
            />
            <ReceiptRow label="Gateway Response" value={receipt.gatewayResponse} />
            <ReceiptRow
              label="Date & Time"
