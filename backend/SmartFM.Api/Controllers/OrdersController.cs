...
public class OrdersController : ControllerBase
{
    ...
    public async Task<ActionResult<OrderDetailsResponse>> GetOrderById(Guid id)
    {
        ...
        return Ok(OrderDetailsResponse.FromEntity(details.Value.Order, details.Value.Shipment, details.Value.Cargoes));
    }

    [HttpGet]
    [ProducesResponseType(typeof(IEnumerable<OrderSummaryResponse>), StatusCodes.Status200OK)]
    public async Task<ActionResult<IEnumerable<OrderSummaryResponse>>> GetOrdersByCustomerEmail([FromQuery, Required, EmailAddress] string customerEmail)
    {
        var orders = await _coordinator.GetOrdersByCustomerEmailAsync(customerEmail);
